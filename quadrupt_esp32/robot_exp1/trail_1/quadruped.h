...
void standPose()
{
  // Smoothly move Hip2 from 80° -> 90°
  // Smoothly move Knee from 120° -> 100°

  for(int step = 0; step <= 20; step++)
  {
    int hipAngle  = 80 + (step * 10) / 20;   // 80 -> 90
    int kneeAngle = 120 - step;              // 120 -> 100

    L1Hip2.write(hipAngle);
    L2Hip2.write(hipAngle);
    L3Hip2.write(hipAngle);
    L4Hip2.write(hipAngle);

    L1Knee.write(kneeAngle);
    L2Knee.write(kneeAngle);
    L3Knee.write(kneeAngle);
    L4Knee.write(kneeAngle);
...
void sitPose()
{
  // Smoothly move Hip2 from 90° -> 80°
  // Smoothly move Knee from 90° -> 120°

  for(int step = 0; step <= 30; step++)
  {
    int hipAngle  = 90 - (step * 10) / 30;  // 90 -> 80
    int kneeAngle = 90 + step;              // 90 -> 120
